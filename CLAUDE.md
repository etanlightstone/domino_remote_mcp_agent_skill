...
This project provides a Claude Code skill for authenticating to Domino Data Lab
and connecting to remote MCP servers running inside Domino. This is so a user working on their remote laptop can sync files and interact with the Domino plaform via the MCP Server.

## Quick Start

1. Run `/domino-auth <domino host URL>` — authenticates via browser, ask user for URL
2. Restart the Claude Code session — the Domino MCP tools will appear
...
- `.claude/skills/domino-auth/scripts/domino_headers.py` — headersHelper that injects Bearer token into MCP requests
- `.mcp.json` — Pre-configured test MCP server (Weather MCP Server on cloud-dogfood)
...
- Access tokens expire in 5 min but the headersHelper auto-refreshes them
